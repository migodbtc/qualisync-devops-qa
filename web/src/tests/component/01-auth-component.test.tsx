...
describe('Login Page', () => {
  ...
  it('stores access token in localStorage on successful login', async () => {
    ...
    fireEvent.change(screen.getByTestId('login-password'), { target: { value: 'correctpassword' } });
  ...
  it('displays an error message returned by the API when submitting empty credentials', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('login-error')).toHaveTextContent('Email and password are required');
    });
  });
...
describe('Register Page', () => {
  ...
  it('displays an error message when form is submitted without agreeing to compliance checkboxes', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('register-error')).toHaveTextContent('You must agree to all compliance checkboxes.');
    });
  });
