...
from resource.auth import (
    hash_password, verify_password, revoke_jti, find_by_jti, create_session, revoke_session,
    unpack_register_payload, unpack_login_payload, validate_register, validate_login, generate_tokens_and_claims
)
...
def test_validate_login():
    ...
    errors = validate_login(
        mock_login_payload["email"],
        mock_login_payload["password"]
    )
    assert errors == []
    # Negative: Email missing
    with pytest.raises(ValueError, match="email and password are required"):
...
def test_generate_tokens_and_claims():
    ...
    with patch("resource.auth.db", mock_db):
        with patch("resource.auth.create_access_token", return_value="access"), \
             patch("resource.auth.create_refresh_token", return_value="refresh"), \
             patch("resource.auth.decode_token", return_value={"jti": "jti", "exp": 9999999999}):
            mock_db.query.return_value.filter_by.return_value.first.return_value = mock_role_admin
    ...
    with patch("resource.auth.db", mock_db):
        with patch("resource.auth.create_access_token", return_value=None), \
             patch("resource.auth.create_refresh_token", return_value=None), \
             patch("resource.auth.decode_token", return_value={"jti": None, "exp": None}):
            with pytest.raises(ValueError, match="Token generation failed: missing values"):
