...
def list_audit_log():
    ...
    try:
        ...
        per_page = int(
            request.args.get("per_page", current_app.config.get("DEFAULT_PER_PAGE", 20))
        )
    except Exception:
    ...
    if page < 1 or per_page < 1 or per_page > MAX_PER_PAGE:
        _log(
            f"ERROR list: page={page} per_page={per_page} out of range (max={MAX_PER_PAGE})"
        )
        ...
        if not allowed:
            try:
                min_role = auth.minimum_role_for_table_action(
                    TABLE, "read", None, None, g.current_user
                )
            except Exception:
                min_role = "unknown"
            cur_role = g.current_user.get("role") if g.current_user else "UNKNOWN"
            _log(
                f"FORBIDDEN list: table={TABLE} user_role={cur_role} required={min_role}"
            )
...
def get_audit_entry(item_id: int):
    ...
    try:
        ...
        allowed = auth.authorize_table_action(
            TABLE, "read", None, item_id, g.current_user
        )
        if not allowed:
            try:
                min_role = auth.minimum_role_for_table_action(
            ...
            cur_role = g.current_user.get("role") if g.current_user else "UNKNOWN"
            _log(
                f"FORBIDDEN get: table={TABLE} id={item_id} user_role={cur_role} required={min_role}"
            )
            return jsonify({"error": "forbidden"}), 403
