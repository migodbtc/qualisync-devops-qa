...
bp = Blueprint(TABLE, __name__, url_prefix="/api/medical_records")
...
def create_medical_record():
    ...
    try:
        ...
        allowed = auth.authorize_table_action(
            TABLE, "create", data, None, g.current_user
        )
        if not allowed:
            try:
                min_role = auth.minimum_role_for_table_action(
                    TABLE, "create", data, None, g.current_user
                )
            except Exception:
                min_role = "unknown"
            ...
            _log(
                f"FORBIDDEN create: table={TABLE} user_role={cur_role} required={min_role}"
            )
...
def list_medical_records():
    ...
    try:
        ...
        _log(
            f"OK list: table={TABLE} page={page} per_page={per_page} returned {len(result.get('items', []))} items"
        )
...
def update_medical_record(item_id: int):
    ...
    try:
        ...
        allowed = auth.authorize_table_action(
            TABLE, "update", data, item_id, g.current_user
        )
        if not allowed:
            try:
                min_role = auth.minimum_role_for_table_action(
            ...
            cur_role = g.current_user.get("role") if g.current_user else "UNKNOWN"
            _log(
                f"FORBIDDEN update: table={TABLE} id={item_id} user_role={cur_role} required={min_role}"
            )
            return jsonify({"error": "forbidden"}), 403
...
def delete_medical_record(item_id: int):
    ...
    try:
        ...
        allowed = auth.authorize_table_action(
            TABLE, "delete", None, item_id, g.current_user
        )
        if not allowed:
            try:
                min_role = auth.minimum_role_for_table_action(
            ...
            cur_role = g.current_user.get("role") if g.current_user else "UNKNOWN"
            _log(
                f"FORBIDDEN delete: table={TABLE} id={item_id} user_role={cur_role} required={min_role}"
            )
            return jsonify({"error": "forbidden"}), 403
