        ...
        def wrapper(*args, **kwargs):
            ...
            if user.get("role") not in roles:
                ...
                try:
                    ...
                    cur_role = "UNKNOWN"
                print(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] FORBIDDEN roles_required: user_id={auth_id} role={cur_role} required={roles}"
                )
...
def minimum_role_for_table_action(
    table: str, action: str, data: dict, item_id: int, current_user: dict
) -> str:
...
def authorize_table_action(
    ...
    if table == "appointments":
        if action == "create":
            ...
            return (
                data.get("patient_id") == uid
                or data.get("staff_id") == uid
                or role in ("doctor", "nurse")
            )
